...
@registry.register_processor("blip_alfworld")
class BlipAlfworldProcessor(BaseProcessor):
    def __init__(self, prompt="", max_words=50):
        self.prompt = prompt
        self.max_words = max_words

    def __call__(self, history, add_prompt=True):
        if add_prompt:
            history = self.prompt + self.pre_process(history)
        else:
            history = self.pre_process(history)
    ...
    def pre_process(self, history):
        history = history.lower()

        # truncate
        history_words = history.split(" ")
        if len(history_words) > self.max_words:
            history = " ".join(history_words[: self.max_words])
