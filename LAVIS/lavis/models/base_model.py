...
class BaseModel(nn.Module):
    ...
    def load_checkpoint_from_config(self, cfg, **kwargs):
        ...
        if load_finetuned:
            ...
            load_basemodel = cfg.get("base_model", False)
            load_pretrained = cfg.get("load_pretrained", True)
            
            if load_basemodel:
                # load base weights
                base_path = cfg.get("base_model", None)
                assert "Found load_base is True, but base_model is None."
                with torch.no_grad():
                    if is_url(base_path):
                        cached_file = download_cached_file(
                    ...
                    logging.info("load base model from %s" % base_path)
            
            if load_pretrained:
                # load pre-trained weights
                pretrain_path = cfg.get("pretrained", None)
                assert "Found load_finetuned is True, but pretrain_path is None."
